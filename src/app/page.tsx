  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Visualize your Google Maps Timeline data with this free tool. Upload your location history and see where you've been." />
        <link rel="canonical" href="https://googlemapstimeline.com" />
      </Head>
      <main className="container mx-auto p-4 md:p-6 min-h-screen">
        <h1 className="text-3xl font-bold mb-6">Google Maps Timeline Visualizer</h1>
        
        {!isFileUploaded ? (
          // Show file upload UI if no file has been uploaded yet
          <div className="my-8">
            <h2 className="text-xl font-semibold mb-4 text-center">Upload Your Location History</h2>
            ...
            <div className="mt-12 max-w-3xl mx-auto">
              ...
              <div className="space-y-6">
                <div className="bg-card p-4 rounded-lg shadow">
                  ...
                  <p>Yes, it is completely free to use! But if you find it helpful, you can donate to support development.</p>
                </div>
                
                <div className="bg-card p-4 rounded-lg shadow">
                  <h3 className="text-lg font-semibold mb-2">🔒 Is my data safe?</h3>
                  <p>Absolutely! We don't store your data at all - we don't even have a backend server. All processing happens locally in your browser.</p>
                </div>
          ...
          <>
            ...
            <div className="stats mb-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              <div className="stat p-4 bg-card rounded-lg shadow">
                <div className="stat-title text-muted-foreground">Total Locations</div>
                <div className="stat-value text-2xl font-bold">{mapMarkers.length}</div>
                <div className="stat-desc text-muted-foreground text-sm">Places visited</div>
              </div>
              <div className="stat p-4 bg-card rounded-lg shadow">
                <div className="stat-title text-muted-foreground">Path Points</div>
                <div className="stat-value text-2xl font-bold">{mapPaths.length}</div>
                <div className="stat-desc text-muted-foreground text-sm">Travel coordinates</div>
              </div>
              <div className="stat p-4 bg-card rounded-lg shadow">
                <div className="stat-title text-muted-foreground">Date Range</div>
                <div className="stat-value text-2xl font-bold">{dateRange ? 
                  (() => {
                    const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
                    const fromMonth = monthNames[dateRange.from?.getMonth() || 0];
                    const fromDate = dateRange.from?.getDate();
                    const toMonth = dateRange.to ? monthNames[dateRange.to.getMonth()] : fromMonth;
                    const toDate = dateRange.to?.getDate() || fromDate;
                    return `${fromMonth} ${fromDate} - ${toMonth} ${toDate}`;
                  })()
                  : "None"}
                </div>
                <div className="stat-desc text-muted-foreground text-sm">Selected period</div>
              </div>
              <div className="stat p-4 bg-card rounded-lg shadow">
                <div className="stat-title text-muted-foreground">Data Points</div>
                <div className="stat-value text-2xl font-bold">{filteredLocations.length}</div>
                <div className="stat-desc text-muted-foreground text-sm">Timeline entries</div>
              </div>
            </div>
            ...
            <div className="mb-6 relative" style={{ height: "600px" }}>
              {filteredLocations.length > 0 ? (
                ...
                <div className="h-full flex items-center justify-center bg-secondary rounded-lg">
                  <p className="text-lg">No location data available for the selected date range</p>
                </div>
              )}
            </div>
          </>
